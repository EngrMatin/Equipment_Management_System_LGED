...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head id="ctl00_Head1">
        ...
        <?php 
            ...
            {
                $contractor_new_password    = $_POST['contractor_new_password'];

				if ($_POST['contractor_password'] != $contractor_password) 
				{
					echo "<script>alert('Wrong Password')</script>";
				}
				else if ($_POST['contractor_new_password'] != $_POST['confirm_new_password']) 
				{
					echo "<script>alert('Passwords do no match')</script>";
				}
				else 
				{
					$sql ="update contractor_info set contractor_password='$contractor_new_password' where contractor_id=$contractor_id";
					$result = $conn->query($sql);

					if($result == 1)
					{
						$message = "Password Changed Successfully";
						header("Location: contractor_account.php?message=".$message);
					}
				}
            }
...
?> 
    ...
    <div id="ctl00_ContentPlaceHolder1_dvEng">
        ...
        <br>
        <div align="center" ><span style='font-size: 20px; color: blue; font-weight: bold;'>Change Password</span></div>
	...
	<script>
      ...
      <form action="change_contractor_password.php" method="post" name="myform">
        <div class="user-details">

          <div class="input-box">
            <span class="details">Old Password</span>
            <input type="password" name="contractor_password" placeholder="Enter old password" required>
          </div>
          
          <div class="input-box">
            <span class="details">New Password</span>
            <input type="password" name="contractor_new_password" placeholder="Enter new password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^\w]).{8,}" title="Password must contain at least one uppercase and one lowercase letter, one special character, one number, and at least 8 characters" required>
          </div>
          <div class="input-box">
            <span class="details">Confirm New Password</span>
            <input type="password" name="confirm_new_password" placeholder="Confirm new password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^\w]).{8,}" title="Password must contain at least one uppercase and one lowercase letter, one special character, one number, and at least 8 characters" required>
          </div>
        </div>
